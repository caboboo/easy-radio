...
目前的 Version 與 Build Number 集中保存在 `version.js`。除非 Project Owner 明確要求升版，否則 Version 保持不變；每次完成一輪修改並準備正式部署時才將 Build Number 加 1，未 Push 的修改不算新 Build。現在是 `Version v0.3`、`Build 041`。
...
- `station-sort.js`：不改動原始資料的電台排序與本機偏好設定
- `station-favorites.js`：以 station ID 保存並共用電台 Favorite 狀態
...
Build 033 在頻率低至高／高至低排序時，將頻率改為電台卡片左側的主要掃描欄位；預設與名稱排序保留原版面，既有排序邏輯、保存偏好、播放與 Greenpeace iframe lifecycle 均未修改。
Build 034 降低 Greenpeace 官方播放器「重新載入」操作的視覺權重，並以播放器異常時使用的 recovery action 呈現；主要收起／顯示操作維持醒目，iframe lifecycle 與播放行為均未修改。
Build 035 更新 Greenpeace 官方播放器的提示文字，使其在 iframe 確認步驟前後都維持正確語意；未加入 iframe 狀態偵測，也未修改播放、iframe lifecycle、recovery control 或電台列表行為。
Build 036 將 Greenpeace 底部控制重新定義為官方播放器的導航／顯示操作，移除 Play 圖示，並依 View 與展開狀態顯示「前往官方播放器」、「官方播放器」或「顯示官方播放器」；一般電台 Play／Pause、iframe audio control 與 lifecycle 均未修改。
Build 037 將 Greenpeace 官方播放器的收起／顯示操作集中至固定 Bottom Control，移除內容區域內重複的 show／hide 按鈕；Current Station 依狀態顯示「收起官方播放器」或「顯示官方播放器」，All Stations 與 Settings 維持「前往官方播放器」，iframe lifecycle 完全不變。
Build 038 將 Greenpeace 官方播放器的 recovery controls 移至 iframe 後方，讓正常 helper 與播放器優先呈現；Reload 行為、iframe lifecycle 與 Bottom Control 均未修改。
Build 039 在 Settings 的固定 Bottom Control 加入向右滑關閉功能，並重用既有 X 按鈕的關閉路徑；原 Bottom Drawer 手勢、控制動作、播放、電台與 iframe lifecycle 均未修改。
Build 040 在 All Stations 與 Current Station 加入共用的 ☆／★ Favorite 控制，並以 localStorage 依 station ID 保存；預設排序改為 Favorite-first，名稱與兩種頻率排序維持原語意，播放、手勢與 Greenpeace iframe lifecycle 均未修改。
Build 041 將 Greenpeace 官方 iframe 預設展開為集中設定的 520px Full-page／Locked POC，並加入低權重的「解鎖／鎖定官方播放器」fallback；切換只改變同一 iframe 的高度與 scrolling presentation，Reload 仍是唯一 destroy／recreate 路徑。
...
新增一般電台時，請在 `stations-data.js` 的 `stations` 陣列加入一筆物件，填入唯一的 `id`、`name`、`brand`、`streamUrl`，並可加入 `subtitle`、`frequency` 與 `keywords`。`streamUrl` 必須是已確認可播放的 HTTPS 正式入口；不得填入猜測、第三方、HTTP 或 redirect 後含 `rj-tok`／`rj-ttl` 的短效網址。綠色和平廣播目前不保存動態 HLS token；官方 `iframeUrl` 播放器使用 Build 027 的持久 Current Station DOM 架構，只在綠色和平是目前電台且進入單一電台畫面時顯示。「所有電台」只顯示一般 station card；切換 View、收起／顯示播放器，或選擇一般電台時，iframe 都只會停放到 viewport 外。再次選回綠色和平會恢復同一個 iframe instance、`src` 與 session；只有使用「重新載入官方播放器」才會銷毀並建立新 iframe。Easy Radio 不讀取或保存 token 字串；iframe 與一般 `<audio>` 的 Audio Ownership／雙音源問題不在本 Build 處理，既有一般電台播放流程維持獨立。
